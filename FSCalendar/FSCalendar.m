...
@property (nonatomic) NSMutableArray *displayedMonth;
...
- (void)initialize
{
	...
	_displayedMonth = [NSMutableArray array];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
	if (!self.floatingMode) {
		...
		// Get number of days in month from the date of the first sunday of the month
		NSIndexPath *index = [NSIndexPath indexPathForRow:7 inSection:section];
		NSDate *firstSundayOfMonth = [self dateForIndexPath:index];
		NSCalendar *c = [NSCalendar currentCalendar];
		NSRange days = [c rangeOfUnit:NSDayCalendarUnit
							   inUnit:NSMonthCalendarUnit
							  forDate:firstSundayOfMonth];
		
		[self.displayedMonth addObject:@([self monthOfDate:firstSundayOfMonth])];
		
		NSDate *firstDateOfMonth = [self beginingOfMonthOfDate:firstSundayOfMonth];
